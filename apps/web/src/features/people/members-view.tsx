...
function OpenTab(properties: { readonly openTab: string | undefined }) {
  const unbuilt = TABS.find((tab) => tab.id === properties.openTab)?.unbuilt;
  if (unbuilt !== undefined)
    return <p className="mt-6 border border-border bg-card p-4">{unbuilt}</p>;
  return properties.openTab === "invitations" ? <InvitationsTab /> : <MembersTab />;
}
