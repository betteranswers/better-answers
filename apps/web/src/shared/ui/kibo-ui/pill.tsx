...
export const PillDelta = ({ className, delta }: PillDeltaProps) => {
  ...
  if (delta > 0) {
    return <ChevronUpIcon className={cn("size-3 text-[var(--status-success-fg)]", className)} />;
  }

  return <ChevronDownIcon className={cn("size-3 text-[var(--status-danger-fg)]", className)} />;
};
