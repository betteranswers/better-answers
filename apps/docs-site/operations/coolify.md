...
Every hostname resolves to the tunnel and the tunnel routes to `http://app:3000` on the platform stack; nothing else is exposed, and the tunnel's last ingress rule is a catch-all `http_status:404`. **Three hostnames** (ADR 0022 as amended by ADR 0034) — the **app** hostname `app.<apex>`, which carries the single-page app, sign-in, consent, `/oauth2/*`, `/.well-known/*`, `/jwks` and the MCP surface at `/mcp` on one origin; the **agent** hostname, routed only to `/agent/v1/*` for the share agent; and the **apex**, which serves concept IRIs and answers 404 on everything (ADR 0002). There is no `mcp.` hostname and no `docs.` hostname; no hostname sits behind Cloudflare Access (Access remains the day-two kill switch for `agent.`).
...
**Rate limiting, by path on the app hostname.** Cloudflare's rules are the first line ahead of the app's own Postgres counters (ADR 0018, T-022), which stay as the second. Matched by path rather than hostname, because everything credential-shaped is now a path on one origin (ADR 0034). The thresholds below are the starting values and are recorded here, beside the tunnel rules, rather than in the private file, because they are a design and not an address; the private file records what the estate is actually set to and when it was last tightened. Each rule counts per client IP and blocks for 10 minutes. Cloudflare Free allows one rule; four are needed, **which is the Pro trigger (Q9): before the first client credential exists, the zone is on Pro** — the wizard's Cloudflare stage refuses to pass its rate-limit step otherwise, and this line is the second place it is written.

| Rule | Path | Threshold | Why |
| --- | --- | --- | --- |
| authorization server | `/oauth2/*` | 20 requests / minute | the unauthenticated half of the OAuth flow; a credential-stuffing run is a burst here |
| discovery | `/.well-known/*` | 60 requests / minute | every client fetches it, no client fetches it often |
| the MCP surface without a bearer | `/mcp` where the `Authorization` header is absent | 30 requests / minute | the pre-flight that draws the 401 is one request per connection; more is a scanner |
| sign-in | `/email-otp/send-verification-otp`, `/sign-in/email-otp` | 5 requests / minute | a code request costs an email; the app's own limiter is the second line |
...
**The uptime check — one, external, on the hostname clients use.** Cloudflare's own Health Checks (Traffic → Health Checks; a Pro feature, which the zone already is) run from outside both boxes against **two paths on the app hostname**, each expecting `200`: `/health` — the app is up and its database answers — and `/.well-known/oauth-protected-resource/mcp` — the authorization server's discovery is served, which is what a connector needs before anything else. Every minute, three failures to go red, alerting the **second channel** through Cloudflare Notifications. It is the one signal that covers VPC 1 from the outside; VPC 2 is covered only by its daily and monthly pings (`RUNBOOK.md` page 1 says so). No health check on a rate-limited path counts against the rules above: Cloudflare's own checks bypass them.
