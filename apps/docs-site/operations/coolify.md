...
**Rate limiting, by path on the app hostname.** Cloudflare's rules are the first line ahead of the app's own Postgres counters (ADR 0018, T-022), which stay as the second. Matched by path rather than hostname, because everything credential-shaped is now a path on one origin (ADR 0034): the authorization server (`/oauth2/*`), discovery (`/.well-known/*`), the MCP surface reached without a bearer (`/mcp` with no `Authorization` header), and the sign-in endpoints (`/email-otp/send-verification-otp`, `/sign-in/email-otp`). Each counts per client IP. **Where each rule sits and its threshold is estate configuration**: the private file's § Ingress records the four rules beside the tunnel rules, with the date each was last tightened (ADR 0027; ticket 77's acceptance line). Cloudflare Free allows one rule and four are needed, **which is the Pro trigger (Q9): before the first client credential exists, the zone is on Pro** — the wizard's Cloudflare stage refuses to pass its rate-limit step otherwise, and this line is the second place it is written.
