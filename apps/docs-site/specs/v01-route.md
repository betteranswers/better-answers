...
**Edges.** Blocked by S1 for `open` by locator and unmapped passages only — the entry step, the walk, reuse, the draft, the model client and the concept page are blocked by nothing on the route (ADR 0016's own words; grill Q16, Q19). Blocks S5, S6 and V1. Its block spec is blocked by **T-113**'s verdict 1 — the write path, the graph door and the cascade under this block's read load.
...
**Edges.** Blocked by nothing on the route: B5 stands, and a section populates from concepts an Editor typed or C1 imports. Blocks S5, S6 and V1 (the Knowledge screen). Its block spec is blocked by **T-113**'s verdict 3 — whether the guides stub's docblock claims the right tables and seams.
...
- **The next session is T-113**, the bounded review below. Then S0 and S1 go to `/to-spec` in one session, about T-006's size, as **two block specs** — `apps/docs-site/specs/s0-<slug>.md` and `s1-<slug>.md`, each with its own status row, so S0 can be *done* while S1 builds (grill Q7). S2 and S3 may be specced in a parallel session — their edges permit it (Q19). Each later block spec is one session and yields roughly five tickets. After T-113, a `/c4-architecture` pass records the shape the review leaves, so the diagrams say what the tree says.
- **A bounded architecture review follows this document** (T-113 on the board), so it has a question: *is what is built the right base for S0 to S7?* Four things have never been reviewed and matter for those blocks — the write path, the graph door and the synchronous cascade under the read load S2 puts on them; the worker's shape as a host for a composed cocoindex pipeline (ADR 0036), before any of it is added; whether the three stub slices' docblocks (sources, guides, answering) claim the right tables and seams for S1 to S3; whether the substrate's twenty-two migrations are the right base for the roughly twenty-two record families still missing from ADR 0014's thirty-seven. Run it as pass 2 was run — independent lenses, each briefed with this document's blocks, none reading another's report; the pass-2 brief is the template. Not repeated: the identity seam, the rules, test quality. The Cubic wiki is the reviewers' orientation and is never cited. Its verdicts are spec edges, not block edges: verdict 2 is S1's, verdict 1 is S2's, verdict 3 is S3's (grill Q10); verdict 4 informs every block that adds a record family.
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| **S0** | knowledge | Personal data withheld at the seam; erasure truthful; the drill fully green | — (its spec after T-113) | — | yes | unspecced — **next**, with S1 | — |
| **S1** | knowledge | One uploaded document to a cited passage, full-text, no embedding; the Sources screen | S0; T-113 verdict 2 (its spec) | T-082 | yes | unspecced — **next**, with S0 | — |
| **S2** | knowledge | Ask answers cited or refuses honestly, entry points full-text over the concept index; the model client; the concept page; the Questions screen | S1 for `open` by locator and unmapped passages only; T-113 verdict 1 (its spec) | — | yes | unspecced — third pick | — |
| **S3** | records | A concept typed or edited; a guide section populated and edited; `concept_owner`; the Knowledge screen | T-113 verdict 3 (its spec) | — | yes | unspecced — **next** | — |
