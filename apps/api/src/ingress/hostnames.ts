 ...
 * first fence and stay so (ADR 0022; recorded in `docs/operations/coolify.md` § Ingress).
 * This is the second, and it is in the app because Better Auth's handler is mounted at
 * the wildcard: every one of its endpoints answers on every hostname the process is
 * given, so a tunnel rule that is one console edit from being wrong is otherwise the
 * only thing between `agent.` and the authorization server.
 *
