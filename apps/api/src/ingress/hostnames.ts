...
export const HOSTNAME_SURFACES: readonly HostnameSurface[] = [
  ...
  },
  {
    ...
    hosts: ["app"],
    reason:
      "The MCP endpoint, discovery, the signing keys and the authorization server itself, on the product's own origin. The protected-resource document's `resource` is `${PUBLIC_URL}/mcp` exactly and every access token's audience equals that string, so the endpoint answers on the origin the authorization server issues from and nowhere else — and that origin is `app.`, because the split onto `mcp.` cost an apex-scoped session cookie sent to every subdomain of the estate for the sake of a separation Cloudflare Access no longer needed. These paths are the catch-all's already; they are named here so a builder reads where the issuer's surface is, and so the day a second hostname carries any of them the change is a diff on this line rather than a silence.",
  },
  {
    paths: ["/consent"],
    hosts: ["app"],
  ...
  },
  {
    ...
    hosts: ["app", "loopback"],
    reason:
      "`app.` because the estate's uptime check reaches it from outside on `app.`'s health and on its protected-resource document — two paths on the one open hostname that serves people and hosts. The loopback because the container's own healthcheck is `wget http://127.0.0.1:3000/health` from inside the container, and a fence that refused it would hold `worker` back for ever. Never on `agent.`, which is open for the share agent's one path and admits no second.",
  },
  {
    paths: [],
    hosts: ["apex"],
    reason:
      "The apex carries nothing. The edge answers it `http_status:404` and names only `/c/`; the origin agrees rather than leaving the edge as the only fence. The empty path set is the entry, so the apex is named in this list rather than missing from it.",
  },
  {
    paths: ["/*"],
    hosts: ["app"],
    reason:
      "Everything else this process answers is Better Auth's own handler at the wildcard with `basePath: '/'` — the session, sign-out, email-code and workspace (`/organization/*`) endpoints — plus `/me`, the tier's cookie-session probe, the SPA's static build and its screens' addresses (`/sign-in`, `/choose-workspace`, everything the shell answers after the authorization server declines), and the tRPC mount. The set cannot be enumerated here: it is the plugin list's, and a Better Auth upgrade adds to it, so an enumerated entry would refuse a path the flow needs the day the library grows one. It is reviewed instead of enumerated: `apps/api/tests/better-auth-endpoints.txt` is what this entry admitted when a human last looked, read from the instance's own endpoint table, and `better-auth-endpoints.test.ts` fails with the added and removed paths named when an upgrade moves it. One hostname carries it because there is one origin and one session: the SPA signs in, picks and resumes the OAuth flow on the origin it was served from. That `agent.` and the apex are absent from it is the whole point of the entry.",
  },
];
