...
ALTER TABLE "workspace_config" ADD CONSTRAINT "workspace_config_workspace_id_workspace_id_fk" FOREIGN KEY ("workspace_id") REFERENCES "public"."workspace"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "member_workspace_id_user_id_uidx" ON "member" USING btree ("workspace_id","user_id");--> statement-breakpoint
...
DROP POLICY "workspace_workspace_isolation" ON "workspace" CASCADE;--> statement-breakpoint
CREATE POLICY "workspace_config_workspace_isolation" ON "workspace_config" AS PERMISSIVE FOR ALL TO public USING ("workspace_config"."workspace_id" = (select current_workspace_id())) WITH CHECK ("workspace_config"."workspace_id" = (select current_workspace_id()));