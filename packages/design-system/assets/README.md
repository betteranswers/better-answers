...
**Empty for now.** The better-answers repository (`better-answers/`, mounted read-only)
contains no logo, wordmark, icon set, illustration or photography — it is pre-build: docs,
ADRs and deployment configuration only.

Nothing was drawn or generated to fill the gap. Wherever a mark would go, the name is set
...
`apps/web`'s Control Centre and in `thumbnail.html`.
