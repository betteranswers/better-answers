...
describe("what the slice's four acts answer", () => {
  it("hands every caller an outcome to read, never one to catch", () => {
    // `open` (T-052) and `find` (T-055) read a store now, so their unions have widened to
    // carry the store's own Error — the shape the convention's rule 3 promised would not
