...
step "Integrations: email to you, AND a SECOND channel (SMS or a chat app) assigned to scheduler, pg-hourly, nightly, drill AND — on the Free path — uptime; a missed backup is not an email to read on Monday. On Pro the same channel takes the Cloudflare health-check notifications instead (stage 5)."
