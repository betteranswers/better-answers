...
# tag equal to the schema package's pin, which a Dockerfile cannot import.
...
# The dockerfile manager sees `FROM` alone, so a version fetched by name ages in silence.
# Each of the two lines below sits directly above its `ARG`, the shape the custom manager matches.
...
# The one service that stays root: cron reads /etc/cron.d as root or not at all. The base
# image's entrypoint is cleared so cron is the command.
USER root
ENTRYPOINT []
CMD ["cron", "-f"]
