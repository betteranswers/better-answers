...
2. **Navigate with jCodeMunch**: your first act in this worktree is `index_folder` on the worktree path, not `resolve_repo` alone — until the worktree is a root of its own, every edit you make is written into the main checkout's index. Then `search_symbols`, `find_references` and `get_file_outline` against the worktree's own id. `Read` a file only to edit it.
3. **GitNexus**: never `analyze` — the runner is absent from a worktree and the index is the orchestrator's, taken at the block's tip. From here the MCP tools read that index: `impact` on every symbol before you change it, `repo: "better-answers"`, including a Python one. HIGH or CRITICAL risk goes in your report before the edit is made, and you stop there.

   Which tool answers which act — and the argument each wants — is the table in `build-loop.md`'s gates. Read it rather than guessing between the two.
