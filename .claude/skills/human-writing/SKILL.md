...
## Sentence length and plain words

Where a project sets no target of its own, sentences average about 15 words. Vary them around that, as *Voice matters* says; the average is what counts.

Turn a sentence that stacks qualifications into bullets:

Before: "The job runs nightly, except on weekends, when it runs once, unless a release is pending, in which case it waits for the release, which can take an hour."

After:

- The job runs nightly.
- On weekends it runs once.
- If a release is pending, it waits for the release first. That can take an hour.

Use the common word over a coined one. When a coined term is needed, define it where it first appears:

Before: "Put the request through the funnel and the triager will pick it up."

After: "Send the request to the shared inbox. Whoever is on support duty that week, the triager, picks it up."
