---
name: webhook-source
description: Implement a new built-in webhook source provider in `front`. Use when adding a webhook provider, including provider research, OAuth prerequisites, provider-specific types and client code, preset registration, UI components, and end-to-end testing.
---
...
For local testing, expose the local app publicly and set `WEBHOOKS_PUBLIC_URL`.
