...
**Where this came from.** The web tier's UX rule carried three numbers and a keyboard clause. The numbers are a product decision that almost nothing measures — one list budget asserted in the browser suite, no budget on the tRPC client — and a number nobody measures is a specification, not a rule (the coding-rules audit of 5 September 2026, T-078). No ADR carried the reader-facing posture, so this one does; the rule keeps the keyboard clause, which a reviewer can check on a screen, and the sentence that a missed budget is a bug.
...
- The web tier's UX rule is the keyboard clause — every common action has a keystroke, `?` lists them, bulk work is select-then-command — and points here for the budget.
