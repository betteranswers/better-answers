...
The whole tree — both tiers, every lift, the deploy unit — is public under Apache-2.0; the hosted service is the product. The `LIFT.md` at the root of each `<tier>/lifts/<name>/` snapshot is named **`THIRD_PARTY_NOTICES.md`** (same contents; the name other engineers recognise), and ~~`[LIFT3]`~~ ADR 0027 fixes which licences may be lifted or depended on at all (the constitution's rule saying so was retired on 2026-08-31; the 2026-09-05 amendment below records it): MIT, BSD, ISC, Apache-2.0, PostgreSQL; GPL/AGPL run-only as a separate unmodified process; nothing from an `ee/` directory, ever.
...
The three rules this ADR and ADR 0027 put into `CODING_RULES.md` — one directory with one `THIRD_PARTY_NOTICES.md`, refresh as a deliberate act, permissive licences only — were removed from the constitution on 31 August 2026 (`64c1e78`) by the owner, deliberately, and are not restored; this amendment is the record the repository's own convention asks for. What binds now: lifting by contract is this ADR's own decision and stands as written; the per-lift file's contents and its contract test are `[APP4]` in `apps/api/CODING_RULES.md`; the licence posture — MIT, BSD, ISC, Apache-2.0 or PostgreSQL, copyleft run-only as a separate unmodified process, nothing from an `ee/` directory — is ADR 0027's own text. The root `THIRD_PARTY_NOTICES.md` aggregate and the `check` licence step went with the family and no task builds them. Where this ADR's earlier amendments cite the retired tags, the citation is struck and pointed here, as the index's convention asks; nothing else changes.
