...
**Publish.** A binding is **published** by one recorded Admin act, separate from sensitivity, from audience and from accepting suggestions; until then its chunks and source entities reach Admins only, in Control Centre's preview. The act's audit row carries the Admin's confirmations — lawful basis recorded, privacy information updated, DPIA reference (research 51). Permission-awareness in v0.1 is three binding fields — `published_at`, `sensitivity`, ~~`audience` (`everyone` or group ids)~~ the audience pair — `audience` ∈ *everyone · groups* beside `audience_groups text[]`, one CHECK tying them (struck 2026-09-08, ADR 0039; the amendment below) — plus the carried per-document `external_access` shape (`is_public = true`, a no-op term), applied as one server-side predicate on every chunk read, every graph read and every traversal hop; the `audience` column ships in the first migration, the group UI does not, and no source-system ACL is synchronised.
...
- Record families: `source_binding`, `connector_run` with outcome rows, `source_document`, `extraction_plan`, `credential` (classes per ADR 0041 (the 2026-09-21 amendment below), including the agent token), the publish act's audit row; no `evidence_snapshot` (ticket 16).
...
The 2026-08-30 amendment above cited two rules of the constitution for the per-lift file and the licence list. Both were retired by the owner on 31 August 2026 (recorded today in ADRs 0005 and 0027); the file's contents are the api's lift-contract rule's and the licence list is ADR 0027's own text. Nothing else in this ADR changes.
