...
## Amendment — 2026-09-22, what "talk only over tRPC" is for (T-230 probe 2; a first-principles pass)

"Talk only over tRPC" names a form. Its function, which ADR 0008 states, is that **everything with a shape crossing between `apps/web` and `apps/api` — a field, an output, a refusal word — is checked by the compiler through the one type that crosses**, so a change on one side fails the other's type-check until fixed. Read the consequence as that function. Two things sit beside tRPC today and stay there because the function does not reach them: the **Better Auth client** — the session, sign-in, sign-out, the workspace, OAuth — which is identity carrying its own typed client, live since T-004 and not arriving with P1; and **bytes**, which have no shape a compiler checks. Bytes in: the S1 upload is a tRPC mutation over `octetInputParser`, the binding's descriptor beside the bytes (ADR 0043, 2026-09-22), and this record gains no exception for it — the spike found tRPC streams an octet body and buffers only multipart. Bytes out: tRPC answers JSON and cannot stream a file, so a download — the exports of the route's stories 11 and 16, which no block carries yet — is a route beside tRPC on the same origin, under the same principal resolution and the same crossing table, opened by the ADR of the block that lands it. A route beside tRPC for a shape tRPC carries is what the function refuses. Everything else stands.
