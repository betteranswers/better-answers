...
| 0033 | The interface is Tailwind v4 with the design system's tokens as its theme through one bridge; shadcn, Kibo UI, AI Elements and better-auth-ui own behaviour, the platform owns meaning; the design system's own components are deleted; the components landed in `apps/web/src/shared/ui/` on 03/09/2026 with four how-it-is-written rules relaxed over that directory alone, and Kibo's `table` rejected as v8-bound |
