...
- **Cut lines for `/to-tickets`**, about five: (1) words first and the four families — `CONTEXT.md`, the migrations, ownership, zero-rows and grant tests, the `redaction` agreement; (2) the seam — the module, the pins, the fixture, the harness, the image test, the measurement, the cocoindex ban; (3) the erasure slice — the request, the map, the routine's steps, the report, the identity arm, the ADR 0020 amendment; (4) the two commands, the drill and restore reorder, the rehearsal's two phases, the ADR 0022 amendment; (5) the finding restore act and the DPIA input's function. `/to-tickets` makes the split.
- **Where block specs live.** The route spec names `apps/docs-site/specs/<block>-<slug>.md`; the docs moved to `docs/specs/` in `529e824` and this is the first block spec written there. The route spec's status table is corrected in the same commit as this file.
