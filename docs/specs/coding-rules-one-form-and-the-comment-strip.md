...
12. As the reviewing agent, I want the tenant-table refusal rule's line — a tenant table, a grant or a definer function ships with the test of what it refuses — to fire on a diff that touches a migration, grant, policy or definer function, so that the security pass has a trigger.
13. As the reviewing agent, I want the refuse-once rule — a value is refused once, where it enters — scoped to one process, so that I never ask for a database or cross-tier guard to be removed.
...
- ~~The ADR and spec files that exist when the test is rewritten are a frozen list that only shrinks; the test does not look inside them for retired tags.~~ Reversed by *The strip reaches the config tree, and the frozen documents give up their tags*: no document is exempt, and the list is gone (2026-09-22).
...
- The rule that comments only the why and the rule that writes a rule tag in a rules file, a review finding or a gate's failure message take their new text in the same change as the strip, so the rules file agrees with the gates that land with it.

### The rules files
- One form: a tagged heading with an imperative title, a body, an optional `Reviewer:` line, a BAD/GOOD snippet where it helps. 80 words of prose a rule, snippets free, the tenant-table refusal rule the first entry of a shrink-only exception list; 2,500 words of prose in the root file. Outside code fences a rules file names identifiers only.
...
- Moves: the keystroke rule (every common action has a keystroke) and the accessibility rule (WCAG 2.2 AA, tested with a keyboard and a screen reader) to the web file, the cocoindex rule (never rebuild what cocoindex provides, never rely on what it does not) to the worker file, and the deploy rule — it owns state on disk, proves every job and wipes staging — to a new rules file beside the deploy configuration, which the agents' entry document names.
- Retired with no stub: the rule that the interface is the test surface and the rule that a test starts a real Postgres and reaches the app through `server.request(...)`, both folded into the test-interface rule — a test crosses the interface a caller crosses; the rule of one logger and one config module, `config.py` the tier's only reader of the environment and `log.py` its only logger; the rule that `check` is one command reporting everything; the rule that types every public boundary, mypy strict over `src` and `tests`; the rule that a branch narrows its tests, never its gates; the UX bar — what is needed first, then more, then the action; and the bundle-alone test — a key, a value or a convention goes into a concept file only if a company with no platform and no guides would still want it there — whose test text moves to the OKF document while the rule that keeps a concept file spec-pure stays. Surviving tags do not renumber. The two TYPES sections become tagged rules in a TYPES family numbered from one, one per imperative.
- Content: the refuse-once rule is one guard per condition inside one process, with the double role check as its snippet. The real-store rule — every store the platform runs is run for real — becomes "Real stores, always": every store the platform runs is real in tests; an in-memory adapter is for a service someone else runs. The factory rule — test state is built through a factory module — says a raw insert appears only inside a factory. The environment rule is stated once and distinguishes reading a setting from passing the environment to a child process, which lives in one named function. The tenant-table refusal rule's adversarial pass becomes its `Reviewer:` line. The mutation-triage rule — triage the nightly mutation summary, never the score — is one line. Text about a seam not yet built goes; the ticket that builds the seam writes its rule.
