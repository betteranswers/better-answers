...
The principal-scoped door rolls its transaction back when the work it ran answers a `Result` that is not ok — one function, shared by the transport's door and the slice's. The four acts whose tail returns the enqueue's word after their rows have landed — keep in text, narrow documents, narrow a binding, reprocess a binding — throw there, as the upload's bind already does. The double unwraps the old behaviour forced on callers go. `result.ts`'s rule 5 is reworded in the same change.
