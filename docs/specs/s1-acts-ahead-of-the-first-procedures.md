...
A good test drives an act, a procedure or a command through the interface its caller uses and asserts what that caller can observe — rows, a refusal and its class, an exit code, a connection count — never internals; real Postgres and a real object store, with our own code never mocked; a provoked failure inside a transaction asserts the transaction's outcome; a pair is checked both ways. No new seam:
