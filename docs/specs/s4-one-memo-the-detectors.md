...
22. As the agent building this, I want the seam to answer one withholding per finding — withheld or left in, the tier, one reason of five in order, and how it was written — so that the text, the `overridden` list and the row's tier are three readings of one record rather than three computations.
