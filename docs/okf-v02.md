...
| Storage, serving, query | Non-goal | Concept index rows, chunks, embeddings and the graph — all derived from the bundle, none a source of truth; the bundle is written only by the api, one commit per act, platform-prepared changes waiting as suggestions | ADR 0012 (ticket 15) |
