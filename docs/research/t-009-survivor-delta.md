...
`kernel/actor.ts:81`, T-088's, where one row also resolved. The other two resolved rows are
`store`'s (T-087's, 163 → 161). Every module but those three has a live set byte-identical to
what the triage walked.
