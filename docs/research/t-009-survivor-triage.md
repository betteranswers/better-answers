...
- 346 (the `sources[]` fallback "filtered by citedSourceOf immediately after") — this is the ticket's named reshape, and it is done: `EdgeSource` now carries `sources: readonly CitedSource[]` instead of raw `frontmatter` (~65-77); a new, exported `citedSourcesOf` (~94-100) is the one place that calls `citedSourceOf` per concept, invoked once by the write path (`concepts/landing.ts`'s `writeConceptDelta` call) for the concept just written, and once per row by the inbound backfill (`writeConceptDelta`'s `naming` query, ~665-676) for the other concepts it re-derives. `referencesOf`'s own `cited === undefined` arm is gone — it reads `concept.sources` directly (~377-408).
