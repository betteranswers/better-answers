...
**A docs-only pull request builds no image at all**, so it neither reads this cache nor writes it. Its `check` takes the docs lane — every changed path ending `.md` — which runs the prose gates in one job, `docs-gates`, and installs neither the builder nor the weights (the process review, 21/09/2026; the steps are in `.github/workflows/check.yml` and `apps/api/tests/docs-lane.test.ts` holds which ones the lane skips). Every sentence here about what a pull request reads is about the full lane.

**Reading needs the container builder as much as writing did.** The daemon's own `docker` driver on a hosted runner (Engine 28.0.4 on `overlay2`, buildx v0.37.0) cannot import this backend: handed `--cache-from type=gha,scope=backup` it logged `ERROR: unknown cache importer: gha`, built all six layers cold and exited 0, while the container builder `docker/setup-buildx-action` creates read six of six and the same driver with no flag read none (run 35602449161, 21/09/2026 — three fresh runners, the question and a control each way). The export on that driver stops the build; the import only wastes it. So the `setup-buildx` step on each `check.yml` leg that builds an image — `full-api` and `full-worker`, and no other, since no other leg builds one — and both suites' refusal of that driver are what keep a pull request's probes warm, and a probe that lost either would stay green. `apps/api/tests/image-job.test.ts` holds the builder and the runtime variables to exactly the legs that carry `IMAGE_PROBE_DEFERRED`.
