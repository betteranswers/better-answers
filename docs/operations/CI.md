...
**The smoke waits for the build it promoted.** `/deploy` is asynchronous, and the api being replaced answers `/health` healthy until the orchestrator swaps the container, so a 200 alone passed against the old build before the new one had started. The smoke runs `deploy/await-release.sh`, which polls `/health` until it answers healthy **and** its `image` is the api digest this run resolved, for six minutes from the first poll to the last — twice the three a fresh pull took end to end — and fails naming both digests. `apps/api/tests/await-release.test.ts` runs it against the api's own `/health`, served locally, and against a stand-in answering a deploy's sequence. A release whose api digest is already the one running, a worker-only rollback say, passes on its first poll: the api answering is the one it promoted, and the worker's digest is not on `/health`. The protected-resource document is read after it.
