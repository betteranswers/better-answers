...
- **An erasure reaches a document only through the map.** The routine suppresses and wipes the documents its map found, and the `source-document` finder answers none, so step 10 runs for no binding on a real request today. The suppression is already the workspace's (T-375), and the seam withholds every exact occurrence of its identifiers on the next run of any binding (T-376). **Planned**: the finder finding the documents that name the subject (T-377).
