...
- **Only a dismissal widens a document, and never past the Admin.** `narrowDocuments` and `narrow` refuse `widening-refused`; a keep lets a span back into the text and lifts no class. No act widens a binding today; T-371 plans one, and T-370 the cascade at publish it builds on (`c4-dynamic-document-to-passage.md`).
...
- **An erasure reaches a document only through the map.** The routine suppresses and wipes the documents its map found, and the `source-document` finder answers none, so step 10 runs for no binding on a real request today. **Planned** (T-366): the finder finds the documents that name the subject, and the suppression becomes workspace-wide, applied by the seam by exact case-folded match of the subject's identifiers — emails, names, other.
