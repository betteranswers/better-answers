...
C4Component
  ...
  Container_Boundary(core, "packages/core") {
    ...
    Component(sources, "sources", "slice", "Bind, publish, narrow; the review acts; reprocess; the DPIA input; passages; the upload sweep")
...
- **T-370 and T-371 give `sources` the class acts** the owner ruled on 24/09/2026. Today the binding stores the class the Admin typed at bind (Restricted and everyone when none is typed); no reader sees an unpublished binding, but the concepts citing it derive from that typed class; `publishBinding` runs no cascade; and no act widens a binding. Planned: an unpublished binding derives as Restricted everywhere, and publish records the class it releases and cascades it (T-370); an Admin's widen act that cascades, before C1 (T-371).
- **T-366 gives `erasure` its documents.** The erasure map's `source-document` finder answers none, so a real request writes no suppression and wipes no binding. Planned: a workspace-wide suppression the seam applies by exact case-folded match of the subject's identifiers — emails, names, other.
