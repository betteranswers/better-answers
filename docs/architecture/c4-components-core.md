...
- **S1 puts `enqueueJobIn(principal, tx, input)` beside the door form**, so the bind act lands the binding, its document, its ledger row and the index job in one transaction and the publish act enqueues nothing; `job` gains a typed `subject_id` and a descriptor per kind from which the CHECKs and the fixture derive, `index` the one kind S1 declares (the S1 spec, 10/09/2026).
