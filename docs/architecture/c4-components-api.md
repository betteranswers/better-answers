...
C4Component
  ...
  Container_Boundary(api, "apps/api") {
    ...
    Component(health, "/health", "Hono route", "Database reachable and the authorization server initialised, or 503, naming the image digest it runs; Docker holds the worker on it and a release waits on the digest")
