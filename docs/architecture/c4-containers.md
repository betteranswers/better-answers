...
- **The web talks tRPC only.** The one exception, invitation-accept on the Better Auth client, arrives with P1. Uploads are a tRPC mutation over `splitLink`, never a second HTTP route (S1; transport work follows the tier's own tRPC skills).
