...
C4Dynamic
  ...
  Rel(sources, postgres, "14. In the publish transaction: published_at on the binding, the ledger row with the totals and the DPIA hash; no chunk, no job")
...
- **The bind act is one transaction, and the publish act enqueues nothing.** The binding, its document, its ledger row and the index job land together through `enqueueJobIn(principal, tx, input)`; the job's `subject_id` names the binding under a per-kind CHECK, so an index job is never about nothing (T-113, owner D3). The chunks exist unpublished from that job, so the publish act stamps the binding alone in its own transaction and is refused until the job has finished — a chunk's visibility is read from its binding, never copied onto its row (ADR 0044); the run's last statement re-copies the binding's fields so a narrowing landed mid-run wins (the S1 spec, 10/09/2026).
