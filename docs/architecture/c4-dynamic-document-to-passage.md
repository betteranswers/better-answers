...
C4Dynamic
  ...
  Rel(sources, postgres, "4. One transaction: source_binding with the class and audience the Admin typed — Restricted and everyone when none is typed — unpublished, source_document, the ledger row")
  ...
  Rel(sources, postgres, "10. published_at and the state on the binding, the ledger row with the finding counts, the DPIA hash and the class and audience it releases, and the cascade to the citing concepts; no chunk, no job")
