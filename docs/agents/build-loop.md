...
- **A hygiene batch is one worktree** (12–13/09/2026). Hygiene tasks that share no file across batches share one worktree, one plan note in the shape of `.scratch/build/H2-tooling-hygiene.md`, one iteration per task with the named suites, the batch's root `check` once at its last iteration, and one verifier; several batches run beside each other off the block tip, and the landing's root `check` is the batch's own when the verifier rebases onto the moved tip before taking it — four batches, sixteen tasks, seven root checks in all. What that shape cost, the same two days: an implementor that forked a child or ended a turn on a background wait was never woken (three stalls, each cleared by hand); a report sent to a ralph instead of main woke the planning ralph into an evaluation beside the dispatched one (one note double-written); and a wait loop written without the bracket blocked every bracketed waiter whose pattern matched its text (three killed). So a brief restates three things this page already carries — spawn nothing, report to `main` by that name, the bracket in `[s]cripts` — because a child reads its brief before it reads the page.
...
- **Scope**: the ticket and nothing beside it. A finding outside the ticket is one line in ralph's report, consolidated at the end of a session by the Orchestrator.
...
A brief is under 2,000 characters. Edges and hazards are one line each, and a rule this page already carries is cited by its heading, never restated.
...
The note carries everything a fresh child would otherwise re-derive: the ticket's acceptance lines verbatim, the rule tags the ticket touches with the sentence each binds, the scouts' findings with their line numbers, and every `impact` reading already taken with its risk — so the next child reads the note and not `ordna show`, the rules file and the graph again. An iteration's focus is sized for one child to finish in about a hundred turns.

## Environment

A Docker daemon (every suite starts a Testcontainers Postgres; T-124 onward a Garage; T-122 and T-129 build and run the worker image), `uv`, `pnpm`, Playwright's browsers for the web, a warm `HF_HOME` for the detector's weights from T-121, and `git-filter-repo` on the path at the version `apps/api/Dockerfile` pins (`uv tool install git-filter-repo==<that version>`; the check workflow reads the same pin) from T-124. The serialisation is a lock no orchestrator turn is spent on, in two steps: an evaluator takes `/tmp/root-check.lock` with `mkdir`, which is atomic — `until mkdir /tmp/root-check.lock 2>/dev/null; do sleep 60; done` — then waits until no root `check` runs — `until ! pgrep -f 'node [s]cripts/check.mjs' >/dev/null; do sleep 60; done` — each in Bash calls of up to ten minutes; runs; and `rmdir`s the lock when the run ends, whatever it read. Two things in that idiom are load-bearing: the bracket, since `pgrep -f` reads the waiting shell's own command line and a bare pattern matches itself forever; and that a waiter never `rmdir`s inside its wait, since the lock it would remove is another's. A lock left by a killed evaluator is removed by hand once `pgrep` answers nothing. The buildx cache is bounded by one command run by hand — `docker buildx prune -a` — after the last root `check` of a session, and never between an image build and the root `check` that reuses it, which would throw away the layers that run is about to read.
