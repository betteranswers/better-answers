...
| A wave that adds a source or a test file | `node scripts/jscpd.mjs` and `pnpm knip`, both from the worktree root |
| The workspace | its `check`; root `pnpm check` once, last |

The named suites are the loop's arbiter, run by the evaluator in its own foreground after every implementor. Root `check` runs once, by the evaluator whose iteration otherwise passes, and again at landing; the whole suite mid-loop is waste. The duplication and dead-code gates are the exception a plan names for itself: both are whole-tree steps of the root `check` and neither takes a scope, so a ticket that adds files otherwise meets them only after every named suite is green, where a fold costs a whole iteration — `packages/core`'s own `check` was green at 800 tests while root `check` was red on 14 clones, every one of them in files that ticket had created (T-131, and T-129 met the same gate at its last iteration, 11/09/2026).
...
- **Navigation is jCodeMunch** (`AGENTS.md`, *Code Exploration Policy*): a worktree's first jCodeMunch act is `resolve_repo` on the worktree path, which must answer an id of the worktree's own and not `betteranswers/better-answers`, because only an index of its own makes the worktree a root — until it has one, every file it edits is registered into the **main checkout's** index at a path under `.claude/worktrees/`, since `index_file` and `register_edit` resolve an edited path to the nearest containing root and consult no ignore list (T-146, 12/09/2026). Provisioning now gives it one: `.claude/hooks/provision-worktree.sh`'s `jcodemunch index` stage indexes the worktree as its own root before an agent's first turn, and the remove hook drops that index again with the worktree (T-181, 12/09/2026). An `index_folder` on the worktree path is what to do when `resolve_repo` answers the main checkout's id anyway — a worktree cut before this landed, or a machine with no jCodeMunch, where provisioning says so on its own line and carries on. The ignore list binds the discovery walk, and it is `.jcodemunch.jsonc` at the repository root, key `extra_ignore_patterns`, gitignore syntax, one pattern — `**/.claude/worktrees/**` — declared whole because the project file overlays the global `~/.code-index/config.jsonc` per key at read time rather than merging with it. What the walk's exclusion buys is a search that stops answering out of another ticket's worktree, or out of one no longer on disk (T-123). `Read` a file only to edit it. A `Read` that pages through a file with moving offsets to find something is the fallback the policy forbids and every page stays in context for the rest of the iteration: search first, then read the section found (250 windowed reads across two tickets, 11/09/2026). This binds ralph's own reading in Phase 1 as much as an implementor's.
- **GitNexus**: the index lives in the main checkout only (`.gitnexus/` is excluded per checkout and the runner is absent from a worktree), so `analyze` is the orchestrator's, at every landing. From a worktree the MCP tools read that index, which is at the block's tip: `impact` before editing any symbol that exists there — a symbol the ticket creates has no caller to report — and `detect_changes` before every commit with the worktree's absolute path passed explicitly. HIGH or CRITICAL risk is reported to ralph before the edit, never absorbed. An `impact` answer of *ambiguous* is not a risk reading: the same symbol read LOW by name and CRITICAL by `target_uid` (T-128, 11/09/2026), so an ambiguous answer is re-run with the uid before it counts. **The worker tier is inside this gate rather than outside it** (T-167, 12/09/2026): `impact` resolves an existing Python symbol from a worktree exactly — `read_bootstrap` reads 3 impacted, 2 direct, LOW, `epistemic: exact`, two affected processes, and by `target_uid` identically — so `apps/worker/` is gated like every other tier and no substitute is owed. T-129's *not found* was `nightly_audit`, a name no symbol in this tree carries; a name the index does not hold is the one thing `impact` cannot answer, and that is a spelling to check rather than a tier to exempt. `detect_changes` takes `repo: "better-answers"` **and** `worktree:` the worktree's absolute path — the worktree path is not itself a `repo`, because `repo` names an indexed index and GitNexus indexes only the main checkout, so passing the path there errors *Repository … not found*.
- **Which tool answers which act** (T-167, 12/09/2026). Both tools answer some of the same questions, and they answer them out of different indexes: jCodeMunch reads the **worktree's own**, which follows the working tree edit by edit, and GitNexus reads the **main checkout's**, which stands at the block's tip until the orchestrator's next `analyze`. So jCodeMunch is asked *where a thing is* and GitNexus *what a change breaks*, and where both would answer the table names one.

  | Act | Tool | From a worktree, how |
  | --- | --- | --- |
  | Find a definition | jCodeMunch `search_symbols` | `repo` is the worktree's own id, which `resolve_repo` on the worktree path returns. It answers with the signature and the line; GitNexus `context` answers too, but adds callers and processes the question did not ask for. |
  | Find references or importers | jCodeMunch `find_references`, or `find_importers` for a whole file | The argument is **`identifier`** (`find_importers` takes `file_path`). `symbol` and `name` are not spellings of it and fail the call rather than answering it. |
  | Read a file's shape | jCodeMunch `get_file_outline` | Signatures and summaries without the bodies. `Read` the file only when you are about to edit it. |
  | A symbol's blast radius | GitNexus `impact`, `direction: "upstream"` | `repo: "better-answers"`, the main index's name. The reading is the block tip's, so a symbol this ticket creates has no caller to report; *ambiguous* is re-run by `target_uid` before it counts (T-128). |
  | A Python symbol's blast radius | GitNexus `impact`, exactly as above | No difference, and no substitute: the worker tier resolves by name and by uid alike (the gate above carries the reading). jCodeMunch `get_blast_radius` answers files rather than risk, so it is not the gate. |
  | Diff-to-flows before a commit | GitNexus `detect_changes` | `repo: "better-answers"` **and** `worktree:` the worktree's absolute path, both. It answers what the diff touched; what depends on that is `impact`'s question, and a run with zero affected processes is not a claim that nothing depends on the symbol. |
  | A full-text search | jCodeMunch `search_text` | It reads the worktree's own index, so it sees this ticket's uncommitted edits, which the main checkout's index cannot. |
  | A search across documents | **Neither — a shell `grep`**, with that reason said out loud | A document's *prose* is in no index. jCodeMunch does not index markdown at all: `get_file_tree` on `docs/agents/` answers an empty tree, and the languages its index reports carry no markdown. GitNexus carries the 92 `.md` files and their **headings** as `File` and `Section` nodes — so `cypher` finds a heading and the line it opens at, and `detect_changes` names the section a commit moved — but never the text under one, and no tool of either offers full-text search over prose. This is the one act where the policy's fallback is the answer rather than the last resort. |
...
- **A rule tag never appears in source, a deploy file, a Dockerfile, a CI workflow or a workspace config** — the scan's own scope, wider than "source": a comment there carries the constraint in words (T-122, T-124, 11/09/2026: three loops lost a root-check run to this). **Nor in a test.** `[COMMENT2]` stopped admitting one on 2026-09-11, and a test says what it holds in words, in its docblock; the 105 tests that still cited a tag are frozen as a baseline in `apps/api/tests/coding-rules-tags.test.ts`, which only shrinks. So a citation in a test the baseline does not list is refused, and a file that sheds its last tag loses its baseline entry in the same commit (T-182, 12/09/2026). Where a tag may still be written is that scan's `isAllowedLocation`, and it is the arbiter.
