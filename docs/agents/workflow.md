...
5. **Land**: push the branch, open the PR, wait for CI — the root `check` runs on every pull request — merge on green, then `node .gitnexus/run.cjs analyze` in the main checkout. Let the `build` run one merge starts finish before the next merge: its group is per commit (`T-211`), so nothing queues them, and two runs at once export to the same cache scopes — safe, and paid for in cache (`docs/operations/BUILD_CACHE.md`, *The runner's cache*).
