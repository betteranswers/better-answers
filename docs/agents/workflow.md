...
It names a branch from the message, commits the working tree over `origin/main`'s head, pushes, opens the pull request with `gh pr create --fill` so its title is the commit's sentence, arms the merge, and prints the pull request number with the queue state read back.

**`pnpm land` is where the message's shape is written**, and this document states no list of its own to go stale: a message it will not take comes back naming the condition it missed, as does a tree it will not land.
