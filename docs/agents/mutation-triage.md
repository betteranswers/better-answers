...
- **`testsCompleted` is nonzero** before a row is called a survivor. A row with `testsCompleted: 0` was never run — the runner resolved its covering tests to nothing. The cause in this tree was vitest's `related` lookup finding no test file for a module only a re-export or a dynamic import reaches, and it is fixed: both `stryker.config.mjs` files set `vitest.related` off (T-107). A zero-test row in a report now is a regression in the runner to report as one — the job summary names such rows under their own heading — never a gap to close and never a mutant to skip: `ignoreStatic` was tried on both legs and measured (T-097's Progress) as 304 kills lost in `apps/api` and 192 phantom survivors made in `packages/core`.
