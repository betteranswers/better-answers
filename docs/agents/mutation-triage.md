...
- **`testsCompleted` is nonzero** before a row is called a survivor. A row with `testsCompleted: 0` was never run — the runner resolved its covering tests to nothing, which in this tree means vitest's `related` lookup found no test file for a module only a re-export or a dynamic import reaches — and it is a runner fault to fix where the runner failed, not a gap to close and not a mutant to skip: `ignoreStatic` was tried on both legs and measured (T-097's Progress) as 304 kills lost in `apps/api` and 192 phantom survivors made in `packages/core`.
