...
- **A whole-suite `killed` is a kill only when the failing test reaches the mutated file.** The api triage's first probe of `auth/auth.ts:254` came back `killed` because `provision-worktree.test.ts`, a git gate with no path to the mutated module, timed out at 60 s; the re-run survived. Read which test failed before writing the verdict, and re-run when it is one the mutation cannot reach.
...
- **`testsCompleted` is nonzero** before a row is called a survivor. A row with `testsCompleted: 0` was never run — the runner resolved its covering tests to nothing. The cause was the runner: a mutant that throws while its module loads — an emptied act family, an emptied MCP entry — fails every test file that imports it before one test runs, and `@stryker-mutator/vitest-runner` read a file with no test tasks as nothing run. It is fixed by the patch under `patches/`, which reports the file's failure to load as the kill it is (T-107; `vitest.related` was read as the cause and is not — `vitest related <file>` names the test files, and off it costs both legs the two-hour timeout). A zero-test row in a report now is a regression in the runner to report as one — the job summary names such rows under their own heading — never a gap to close and never a mutant to skip: `ignoreStatic` was tried on both legs and measured (T-097's Progress) as 304 kills lost in `apps/api` and 192 phantom survivors made in `packages/core`.
