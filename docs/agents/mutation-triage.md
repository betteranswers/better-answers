...
- **`testsCompleted` is nonzero** before a row is called a survivor. A row with `testsCompleted: 0` was never run — the runner resolved its covering tests to nothing — and the nightly fails the leg that holds one, as a runner fault. Both causes so far were the runner, and the patch under `patches/` fixes both: a mutant that throws while its module loads — an emptied act family, an emptied MCP entry — fails every test file that imports it before one test runs, which the patch reports as the kill it is; and vitest 5's name filter, which selected no test for any mutant until the patch built names the way it matches them (`docs/operations/CI.md`, *A runner fault fails the leg*).

Equivalence has two classes, and only one is a proof. *Behaviourally identical, argued from the code* — the mutant cannot produce a different statement, row or value. *Nothing observable at this seam* — the two functions differ, and no caller, driver or fixture in this tree can reach the difference. Label the second as the second: a test written with a bespoke fixture purely to move the number is the report inverting the rule, and the mutation rule says a falling score is never a gate.
